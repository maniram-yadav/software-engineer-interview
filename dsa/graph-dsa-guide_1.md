...
## Section 1-3: Topological Sorting

### 1. Course Schedule

**LeetCode:** https://leetcode.com/problems/course-schedule

**Problem:** `numCourses` courses labeled `0` to `n-1`. `prerequisites[i] = [a, b]` means you must take course `b` before course `a`. Return `true` if you can finish all courses (i.e., no cycle in the dependency graph).
...
**Approach:** Cycle detection in a directed graph. Kahn's BFS (topological sort by removing indegree-0 nodes) or DFS with a 3-color state (white/gray/black) to detect back-edges.

#### Brute Force: Repeated full-array scanning for zero-indegree nodes (no queue)
...
class Solution {
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int round = 0; round < numCourses; round++) {
            ...
            removed[found] = true;
            removedCount++;
            for (int next : graph.get(found)) indegree[next]--;
        }
...
#### Optimal: Kahn's Algorithm (BFS with queue)
...
#### Optimal: DFS 3-color cycle detection
...
class Solution {
    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for (int i = 0; i < numCourses; i++) {
            if (color[i] == WHITE && hasCycle(i, graph, color)) return false;
        }
        return true;
    }
    ...
    private boolean hasCycle(int node, List<List<Integer>> graph, int[] color) {
        ...
        for (int next : graph.get(node)) {
            ...
            if (color[next] == WHITE && hasCycle(next, graph, color)) return true;
        }
        color[node] = BLACK;
        return false;
    }
...
**Problem:** Same setup as Course Schedule, but return a valid course order to finish all courses, or an empty array if impossible.
...
**Approach:** Standard topological sort, recording the order as nodes are processed.

#### Brute Force: Repeated full scanning (no queue)
...
**Problem:** DAG with `n` nodes `0..n-1`, given as adjacency list `graph[i]` = nodes reachable directly from `i`. Return all possible paths from node `0` to node `n-1`.
...
**Approach:** Since we need to enumerate *all* paths, this is inherently exponential in the worst case. Both "brute" and "optimal" are DFS backtracking; the difference is whether we needlessly recompute/copy state.

#### Brute Force: DFS with new list copy at every recursive call
...
class Solution {
    public List<List<Integer>> allPathsSourceTarget(int[][] graph) {
        ...
        dfs(0, graph, new ArrayList<>(List.of(0)), result);
        return result;
    }
    ...
    private void dfs(int node, int[][] graph, List<Integer> path, List<List<Integer>> result) {
        ...
        for (int next : graph[node]) {
            ...
            newPath.add(next);
            dfs(next, graph, newPath, result);
        }
    }
}
```
**Complexity:** `O(2^V * V)` worst case, with extra `O(V)` copy cost at every branch.

#### Optimal: DFS with backtracking (mutate + undo)
...
```
**Complexity:** `O(2^V * V)` worst case (inherent to enumerating all paths), but minimal overhead. Space `O(V)` recursion depth + output size.
...
**Problem:** Directed graph with `n` nodes. A node is "safe" if every possible path starting from it eventually leads to a terminal node (no cycles reachable from it). Return all safe nodes in ascending order.
...
**Approach:** A node is unsafe if it's part of a cycle or can reach a cycle. Use DFS with 3-color marking, or reverse the graph and run Kahn's BFS from terminal (outdegree-0) nodes.

#### Brute Force: DFS per node without memoization
...
class Solution {
    ...
    private boolean isSafe(int node, int[][] graph, Set<Integer> visiting) {
        ...
        visiting.add(node);
        for (int next : graph[node]) {
            if (!isSafe(next, graph, new HashSet<>(visiting))) {
                return false;
            }
        }
        return true;
...
```
**Complexity:** `O(V * (V+E))` — no memoization, plus `O(V)` set-copy overhead per call.

#### Optimal: DFS with 3-color memoization

```java
import java.util.*;

class Solution {
    private static final int UNVISITED = 0, VISITING = 1, SAFE = 2, UNSAFE = 3;
    ...
    public List<Integer> eventualSafeNodes(int[][] graph) {
        ...
        for (int i = 0; i < n; i++) {
            if (dfs(i, graph, state) == SAFE) result.add(i);
        }
        return result;
    }
    ...
    private int dfs(int node, int[][] graph, int[] state) {
        if (state[node] != UNVISITED) {
            return state[node] == VISITING ? UNSAFE : state[node];
        }
        state[node] = VISITING;
        for (int next : graph[node]) {
            if (dfs(next, graph, state) == UNSAFE) {
                state[node] = UNSAFE;
                return UNSAFE;
            }
        }
...
#### Optimal Alternative: Reverse graph + Kahn's BFS from terminal nodes
...
class Solution {
    public List<Integer> eventualSafeNodes(int[][] graph) {
        ...
        for (int i = 0; i < n; i++) if (outdegree[i] == 0) queue.add(i);
...
**Problem:** `n` items, `m` groups. `group[i]` is item `i`'s group (`-1` if ungrouped). `beforeItems[i]` lists items that must come before item `i`. Return a valid item order respecting both item-level dependencies and group contiguity, or an empty array if impossible.

**Example:**
```
n = 8, m = 2, group = [-1,-1,1,0,0,1,0,-1]
beforeItems = [[],[6],[5],[6],[3,6],[],[],[]]
Output: [6,3,4,1,5,2,0,7] (one valid ordering)
```

**Approach:** Two-level topological sort: assign ungrouped items unique group IDs, build item-level and group-level graphs, topologically sort groups then items within each group, concatenate.

#### Brute Force: Naive repeated-scan topological sort (no queue) at both levels
...
class Solution {
    public int[] sortItems(int n, int m, int[] group, List<List<Integer>> beforeItems) {
        int groupId = m;
        int[] g = group.clone();
        for (int i = 0; i < n; i++) if (g[i] == -1) g[i] = groupId++;
        ...
        for (int i = 0; i < n; i++) {
            for (int before : beforeItems.get(i)) {
                ...
                itemIndeg[i]++;
                if (g[before] != g[i]) {
                    groupGraph.get(g[before]).add(g[i]);
                    groupIndeg[g[i]]++;
                }
            }
        }

        int[] groupOrder = naiveTopoSort(groupGraph, groupIndeg, groupId);
        if (groupOrder.length == 0) return new int[0];

        int[] itemOrder = naiveTopoSort(itemGraph, itemIndeg, n);
        if (itemOrder.length == 0) return new int[0];
        ...
        for (int grp : groupOrder) {
            for (int item : itemsByGroup.getOrDefault(grp, new ArrayList<>())) {
                result[idx++] = item;
            }
        }
        return result;
    ...
    private int[] naiveTopoSort(List<List<Integer>> graph, int[] indeg, int n) {
        ...
        for (int round = 0; round < n; round++) {
            int found = -1;
            for (int i = 0; i < n; i++) {
                if (!removed[i] && indegCopy[i] == 0) { found = i; break; }
            }
            if (found == -1) return new int[0];
            removed[found] = true;
            order[idx++] = found;
            for (int next : graph.get(found)) indegCopy[next]--;
        }
        return order;
    }
}
```
**Complexity:** `O(n^2 + m^2 + E)`.

#### Optimal: Kahn's BFS at both levels
...
class Solution {
    ...
    private List<Integer> topoSort(List<List<Integer>> graph, int[] indeg, int n) {
        Deque<Integer> queue = new ArrayDeque<>();
...
```
**Complexity:** `O(V + E)` at each level → overall `O(n + m + E)`. Space `O(n + m + E)`.

---

### 6. All Ancestors of a Node in a Directed Acyclic Graph

**LeetCode:** https://leetcode.com/problems/all-ancestors-of-a-node-in-a-directed-acyclic-graph

**Problem:** DAG with `n` nodes and `edges`. For each node, return the sorted list of all its ancestors.
...
**Approach:** Check reachability for every pair (brute), or propagate ancestry forward via DFS per source (optimal).

#### Brute Force: Pairwise reachability via BFS/DFS
...
class Solution {
    public List<List<Integer>> getAncestors(int n, int[][] edges) {
        ...
        for (int target = 0; target < n; target++) {
            ...
            for (int src = 0; src < n; src++) {
                ...
                if (canReach(src, target, graph, new boolean[n])) {
                    ancestors.add(src);
                }
            }
    ...
    private boolean canReach(int src, int target, List<List<Integer>> graph, boolean[] visited) {
...
```
**Complexity:** `O(V * V * (V + E))`.

#### Optimal: DFS from each source, propagating ancestry forward
...
class Solution {
    public List<List<Integer>> getAncestors(int n, int[][] edges) {
        ...
        for (int i = 0; i < n; i++) ancestorSets.add(new TreeSet<>());

        for (int src = 0; src < n; src++) {
            boolean[] visited = new boolean[n];
            dfs(src, src, graph, ancestorSets, visited);
        }

        List<List<Integer>> result = new ArrayList<>();
        for (Set<Integer> set : ancestorSets) result.add(new ArrayList<>(set));
        return result;
    }

    private void dfs(int src, int node, List<List<Integer>> graph,
                      List<Set<Integer>> ancestorSets, boolean[] visited) {
        visited[node] = true;
        for (int next : graph.get(node)) {
            if (!visited[next]) {
                ancestorSets.get(next).add(src);
                dfs(src, next, graph, ancestorSets, visited);
            }
        }
    }
}
```
**Complexity:** `O(V * (V + E))` worst case, but much smaller constant factor than pairwise reachability. Space `O(V^2)`.
...
**Problem:** `numCourses` courses, `prerequisites[i] = [a,b]` meaning `a` is a prerequisite of `b` (direct). Given `queries[j] = [u,v]`, determine for each query if `u` is a prerequisite of `v` (direct or indirect).
...
**Approach:** Precompute the full transitive closure once, answer queries in `O(1)`.

#### Brute Force: Fresh BFS/DFS per query
...
class Solution {
    public List<Boolean> checkIfPrerequisite(int numCourses, int[][] prerequisites, int[][] queries) {
        ...
        for (int[] q : queries) {
            result.add(canReach(q[0], q[1], graph, new boolean[numCourses]));
        }
        return result;
    }
...
#### Optimal: Precompute full reachability matrix (topological DP)
...
class Solution {
    public List<Boolean> checkIfPrerequisite(int numCourses, int[][] prerequisites, int[][] queries) {
        ...
        int[] indegCopy = indegree.clone();
        Deque<Integer> q2 = new ArrayDeque<>();
        for (int i = 0; i < numCourses; i++) if (indegCopy[i] == 0) q2.add(i);
        while (!q2.isEmpty()) {
            int cur = q2.poll();
            topoOrder.add(cur);
            for (int next : graph.get(cur)) if (--indegCopy[next] == 0) q2.add(next);
        }
        ...
        for (int cur : topoOrder) {
            for (int next : graph.get(cur)) {
                ...
                for (int i = 0; i < numCourses; i++) {
                    if (reachable[i][cur]) reachable[i][next] = true;
                }
            }
        }
...
```
**Complexity:** `O(V*E)` to build closure, `O(1)` per query. Total: `O(V*E + Q)`. Space `O(V^2)`.

---

### 8. Strange Printer II

**LeetCode:** https://leetcode.com/problems/strange-printer-ii

**Problem:** `n x m` grid `targetGrid`, colors `1..60`. The strange printer prints one solid-color rectangle per operation, later prints covering earlier ones. Determine if it's possible to reproduce `targetGrid`.
...
**Approach:** For each color, find its bounding rectangle. Every other-color cell inside must have been printed before this color (dependency edge `otherColor -> thisColor`). Check for cycles via topological sort.

#### Brute Force: Naive edge building + naive scan-based topo sort
...
class Solution {
    public boolean isPrintable(int[][] targetGrid) {
        ...
        for (int color : colors) {
            for (int r = minRow[color]; r <= maxRow[color]; r++) {
                for (int c = minCol[color]; c <= maxCol[color]; c++) {
                    ...
                    if (other != color && !edgeSet.contains((long) other * 100 + color)) {
                        edgeSet.add((long) other * 100 + color);
                        edges.add(new int[]{other, color});
                        indegree[color]++;
                    }
                }
            }
        ...
        for (int round = 0; round < colors.size(); round++) {
            int found = -1;
            for (int color : colors) {
                if (!removed[color] && indegree[color] == 0) { found = color; break; }
            }
            if (found == -1) return false;
            removed[found] = true;
            removedCount++;
            for (int[] e : edges) {
                if (e[0] == found) indegree[e[1]]--;
            }
        }
        return removedCount == colors.size();
    }
}
```
**Complexity:** `O(61 * rows * cols + 61^3)`.

#### Optimal: Adjacency set + Kahn's BFS
...
class Solution {
    public boolean isPrintable(int[][] targetGrid) {
        ...
        Arrays.fill(maxRow, Integer.MIN_VALUE);
        Arrays.fill(maxCol, Integer.MIN_VALUE);
        boolean[] present = new boolean[61];
        ...
        int[] indegree = new int[61];
        for (int i = 1; i <= 60; i++) graph[i] = new HashSet<>();
        ...
        for (int color = 1; color <= 60; color++) {
            ...
            for (int r = minRow[color]; r <= maxRow[color]; r++) {
                for (int c = minCol[color]; c <= maxCol[color]; c++) {
                    int other = targetGrid[r][c];
                    if (other != color && graph[other].add(color)) {
                        indegree[color]++;
                    }
                }
            }
...
```
**Complexity:** `O(61 * rows * cols)` for bounding boxes + edges, plus `O(60 + E)` for topo sort. Space `O(61^2)`.

---

### 9. Parallel Courses III

**LeetCode:** https://leetcode.com/problems/parallel-courses-iii

**Problem:** `n` courses, `time[i]` = months to complete course `i`, `relations[j] = [prev, next]`. Courses can run in parallel, but a course starts only after all prerequisites finish. Return the minimum months to complete all courses.
...
**Approach:** Longest path in a weighted DAG. `finishTime[v] = max(finishTime[u] for u in prereqs of v) + time[v]`, computed in topological order.

#### Brute Force: Plain recursive DFS without memoization
...
class Solution {
    ...
    public int minimumTime(int n, int[][] relations, int[] time) {
        ...
        for (int i = 0; i <= n; i++) reverseGraph.add(new ArrayList<>());
        for (int[] r : relations) reverseGraph.get(r[1]).add(r[0]);
    ...
    private int dfs(int course) {
        int maxPrereqTime = 0;
        for (int prereq : reverseGraph.get(course)) {
            maxPrereqTime = Math.max(maxPrereqTime, dfs(prereq));
        }
        return maxPrereqTime + time[course - 1];
    }
}
```
**Complexity:** `O(2^V)` worst case — no memoization causes repeated recomputation of shared subpaths.

#### Optimal: Kahn's topological BFS with DP
...
class Solution {
    public int minimumTime(int n, int[][] relations, int[] time) {
        ...
        while (!queue.isEmpty()) {
            int cur = queue.poll();
            maxTime = Math.max(maxTime, finishTime[cur]);
            for (int next : graph.get(cur)) {
                finishTime[next] = Math.max(finishTime[next], finishTime[cur] + time[next - 1]);
                if (--indegree[next] == 0) queue.add(next);
            }
        }
        return maxTime;
...
**Problem:** `recipes[i]` can be made if all of `ingredients[i]` are available — either in `supplies`, or themselves a recipe that can be made. Return all recipes that can eventually be made.
...
**Approach:** Model as a dependency graph; a recipe's indegree = ingredients that are other recipes. Kahn's BFS from recipes whose ingredients are all available.

#### Brute Force: Fixed-point iteration — repeatedly rescan all recipes

```java
import java.util.*;

class Solution {
    public List<String> findAllRecipes(List<String> recipes, List<List<String>> ingredients, String[] supplies) {
        ...
        while (changed) {
            ...
            for (int i = 0; i < recipes.size(); i++) {
                ...
                if (made.contains(recipe)) continue;
                boolean canMake = true;
                for (String ing : ingredients.get(i)) {
                    if (!available.contains(ing) && !made.contains(ing)) {
                        canMake = false;
                        break;
                    }
                }
        ...
        return new ArrayList<>(made);
    }
}
```
**Complexity:** `O(R^2 * K)`.

#### Optimal: Kahn's BFS on the recipe dependency graph
...
**Problem:** Build a `k x k` matrix containing `1..k` exactly once each, satisfying `rowConditions[i] = [a,b]` (a above b) and `colConditions[i] = [a,b]` (a left of b). Return the matrix, or empty if impossible.

**Example:**
```
k = 3, rowConditions = [[1,2],[3,2]], colConditions = [[2,1]]
Output: [[3,0,0],[1,0,0],[0,2,0]] (one valid answer)
```

**Approach:** Topologically sort the row-order graph and independently the column-order graph. If either has a cycle, return empty. Place `value v` at `(rowIndex[v], colIndex[v])`.

#### Brute Force: Naive scan-based topological sort for both axes
...
class Solution {
    public int[][] buildMatrix(int k, int[][] rowConditions, int[][] colConditions) {
        int[] rowOrder = naiveTopoSort(k, rowConditions);
        if (rowOrder == null) return new int[0][0];
        int[] colOrder = naiveTopoSort(k, colConditions);
        if (colOrder == null) return new int[0][0];

        int[] rowIndex = new int[k + 1], colIndex = new int[k + 1];
        for (int i = 0; i < k; i++) rowIndex[rowOrder[i]] = i;
        for (int i = 0; i < k; i++) colIndex[colOrder[i]] = i;

        int[][] matrix = new int[k][k];
        for (int v = 1; v <= k; v++) {
            matrix[rowIndex[v]][colIndex[v]] = v;
        }
        return matrix;
    }

    private int[] naiveTopoSort(int k, int[][] conditions) {
        List<List<Integer>> graph = new ArrayList<>();
...
```
**Complexity:** `O(k^2 + rowE + colE)`.

#### Optimal: Kahn's BFS for both axes
...
```
**Complexity:** `O(k + rowE + colE)`. Space `O(k + E)`.

---

*More sections (Union-Find, Bipartite, Dijkstra, Bellman-Ford, Floyd-Warshall, and advanced topics) to be added as we cover them.*
